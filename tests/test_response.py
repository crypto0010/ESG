...
# Phrases that state, unambiguously, that an item is not delivered. Either it
# is still outstanding ("not yet", "pending") or it can never be delivered
# ("unrecoverable", "cannot be"). R1.4/R2.4 moved from the first kind to the
# second once the search reconstruction was attempted and failed; both remain
# undelivered, which is what this guard exists to keep the response honest about.
NOT_DELIVERED_MARKERS = (
    "not yet", "pending", "unrecoverable", "cannot be", "could not",
    "not resolved", "unresolved",
)


def test_pending_items_are_not_claimed_as_resolved():
    """R1.3, R1.4, R2.2 and R2.4 are not delivered -- the response must say
    so plainly, and must not imply completion."""
    for item_id in ["R1.3", "R1.4", "R2.2", "R2.4"]:
        item = next(i for i in response_items.ITEMS if i["id"] == item_id)
        low = item["response"].lower()
        assert any(m in low for m in NOT_DELIVERED_MARKERS), (
            f"{item_id} must state plainly that it is not delivered"
        )
        # and must never open by announcing plain resolution
        assert not low.lstrip().startswith("resolved"), (
            f"{item_id} opens by claiming resolution"
        )
